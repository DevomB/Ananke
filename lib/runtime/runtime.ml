...
module Make (D : Domain.S) = struct
  ...
  let check_invariants config state =
  ...
  let maybe_snapshot t =
    if not t.config.snapshot_each_command then t
    else (
      let snapshot =
        Snapshot.create Snapshot_version.current t.clock t.event_index
  ...
  let step t command =
    ...
    let cmd_record =
      Command.create id t.clock ([%sexp_of: D.command] command)
    in
    ...
    | Ok (new_state, emitted_events) ->
        ...
        (match check_invariants t.config t.state with
         ...
         | Ok violations ->
             ...
             in
             let trace =
               Trace.set_final_state ([%sexp_of: D.state] t.state) trace |> Trace.seal
             in
             Ok
               { Transition_result.state = t.state
               ; emitted = List.map emitted_events ~f:[%sexp_of: D.event]
               ; trace
               ; metrics = t.metrics
  ...
  let run t commands =
    let rec loop t = function
      | [] ->
          (* wall_time_ns reserved for a future Core-backed metrics hook *)
