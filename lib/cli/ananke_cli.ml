...
let run domain scenario_path seed output =
  ...
  | Ok scenario ->
    (match String.lowercase domain with
     ...
     | "matching_engine" -> run_matching_engine scenario_path scenario seed output
...
let snapshot_for_domain
  ...
  | None ->
    ...
    (match parse_commands D.command_of_sexp sexps with
     | Error err -> Error err
     | Ok commands ->
       (match R.create config |> fun rt -> R.run rt commands with
        | Error err -> Error err
        | Ok result ->
          ...
          in
          let state = D.sexp_of_state result.state in
          Ok
            (Snapshot.create
               Snapshot_version.current
               clock
               (Event_index.of_int at_index)
               state)))
;;
...
let snapshot trace_path at_index out =
  ...
  | Ok trace ->
    ...
    let snap_result =
      match String.lowercase trace.metadata.domain with
      | "elevator" -> snapshot_for_domain (module Elevator) trace config at_index
      | "ledger" -> snapshot_for_domain (module Ledger) trace config at_index
      | "matching_engine" ->
        snapshot_for_domain (module Matching_engine) trace config at_index
      | other -> failwith (sprintf "unknown domain in trace: %s" other)
    in
...
let count_events (trace : Trace.t) =
  ...
  List.iter trace.events ~f:(function
    ...
    | Event.System Invariant_checked -> Int.incr invariant_checked
...
let report_run (type cmd) (module D : Domain.S with type command = cmd) (trace : Trace.t) =
  let module R = Runtime.Make (D) in
...
let report trace_path format =
  ...
  | Ok trace ->
    let metrics, violations =
      match String.lowercase trace.metadata.domain with
      | "elevator" ->
        (match report_run (module Elevator) trace with
         | Error err -> failwith (Ananke_error.to_string err)
         | Ok pair -> pair)
      | "ledger" ->
        (match report_run (module Ledger) trace with
         | Error err -> failwith (Ananke_error.to_string err)
         | Ok pair -> pair)
      | "matching_engine" ->
        (match report_run (module Matching_engine) trace with
         | Error err -> failwith (Ananke_error.to_string err)
         | Ok pair -> pair)
      | other -> failwith (sprintf "unknown domain in trace: %s" other)
    in
...
let bench_elevator iterations =
  ...
  | Error err -> failwith (Ananke_error.to_string err)
  | Ok _ ->
    let sec = Unix.gettimeofday () -. start in
    if Float.(sec <= 0.)
...
let replay_term =
  let trace =
    Arg.(
      required & pos 0 (some string) None & info [] ~docv:"TRACE" ~doc:"trace sexp file")
  in
