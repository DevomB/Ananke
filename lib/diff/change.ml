...
let describe = function
  | Added (path, value) ->
      sprintf "added %s = %s" (Field_path.to_string path) (Sexp.to_string_hum value)
  | Removed (path, value) ->
      sprintf "removed %s (was %s)" (Field_path.to_string path) (Sexp.to_string_hum value)
  | Changed (path, old_v, new_v) ->
      sprintf "changed %s: %s -> %s"
