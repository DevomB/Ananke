  ...
  | Request_floor floor -> (
      ...
      | Ok () ->
          ...
          let events =
            ...
            @ (if not (equal_direction direction state.direction) then
...
let no_empty_travel state =
  ...
  | Idle -> Ok ()
  | Up ->
      if List.exists state.requests ~f:(fun f -> f > state.floor) then Ok ()
      else
        Error
...
let floor_valid state =
  if state.floor >= min_floor && state.floor <= max_floor then Ok ()
  else
    Error
