...
let matching_engine_scenario_path = "../../examples/matching_engine/scenarios/basic.sexp"
...
let run_save_replay_verify
  ...
  | Error _ -> false
  | Ok result ->
    let path = Stdlib.Filename.temp_file "ananke" ".trace.sexp" in
    Exn.protect
...
let%test "run save replay verify" = run_save_replay_verify (module E) scenario_path
